## Методика тестирования производительности дисковой подсистемы виртуальных машин (VM) на локальном и сетевом iSCSI хранилищах
...
## 	Цель: Сравнить производительность дисковой подсистемы и СУБД PostgreSQL 17 при размещении VM на локальном RAID10 (Dell R750) хранилище и на внешнем iSCSI сетевом хранилище, подключённом по 25GE/100GE, в среде VMware vSphere 8.

## 1. Общие положения
Тестирование проводится на одном хосте ESXi в кластере Т4 дата центра ВКС.

Сравниваются производительности дисковых подсистем VM для двух типов хранилищ:
+ Локальный: VMFS на RAID10, собранный из 8 локальных дисков на хосте Dell PowerEdge R750.
+ Сетевой: VMFS на выделенном LUN iSCSI-СХД (без фоновой нагрузки).

Все VM идентичны и работают под управлением ОС Debian 12.

Тесты включают:
+ Низкоуровневую нагрузку (fio, 60 сек на каждый этап теста)
+ Прикладную нагрузку PostgreSQL 17 (pgbench)
...
+ Запрашивает у пользователя:
  - количество VM для теста и их IP-адреса
  - тип теста (только fio, только pgbench, или оба)
  - количество итераций (рекомендуется 3 для статистической значимости)
  - параметры fio (опционально, можно использовать значения по умолчанию)
+ Автоматически выполняет следующие действия:
  - копирует скрипт `test_fio_7.py` на все тестовые ВМ
  - очищает предыдущие результаты перед каждой итерацией
  - запускает тесты с прогресс-баром отображения
  - собирает результаты в структурированные папки
+ После завершения предоставляет команды для агрегации и визуализации результатов
...
### Участники
+ **Контрольный хост**: MacBook Pro (или любой Linux-хост c возможностью запуска bash и python скриптов)  
...
Перед запуском тестов на всех VM необходимо провести проверку готовности к тестам с помощью скрипта:
```bash
./scripts/verify_test_env.sh
```
Указанный скрипт проверит:
+ Версию PostgreSQL
+ Параметры производительности
+ Файловую систему
+ Доступность по ssh без указания пароля (по ключу)
...
### 8.1. Подготовка iSCSI-хранилища:

  + Настройка MTU 9000 на всех сетевых интерфейсах (ESXi, СХД, коммутаторы)
  + Создание отдельного LUN на СХД без фоновой нагрузки
  + Настройка многоканальности (multipathing) при поддержке
  + Проверка скорости сети с помощью iperf3

### 8.2. Создание iSCSI datastore на ESXi с параметрами:
  + Block size: 1MB (для больших операций)
...
После выполнения всех тестовых этапов результаты обрабатываются в три этапа:

### 9.1 Агрегация результатов
```bash
...
# ... для всех директорий с результатами
```
Скрипт `aggregate_results.py`:
+ Автоматически находит все результаты в поддиректориях
+ Вычисляет средние значения и стандартные отклонения для каждой метрики
+ Формирует отчёты в форматах JSON и TXT
...
Скрипт `visualize_results.py` создаёт в папке `visualization_output/`:
+ Графики сравнения IOPS, Bandwidth и Latency между конфигурациями
+ Графики TPS и задержек pgbench
+ График масштабируемости при увеличении количества ВМ

### 9.3 Интерпретация результатов

При анализе результатов обращаем внимание на:

+ Статистическую значимость (стандартное отклонение не должно превышать 5-10%)
+ Масштабируемость (линейный рост производительности при увеличении ВМ)
+ Отношение цены к производительности для каждого типа хранилища
